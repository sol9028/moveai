...
→ output/ 에 CSV 4종 저장. 시각화는 notebooks/curtailment_viz.ipynb 에서.
...
FORECAST_DIR = Path(__file__).resolve().parent  # forecast/: 입력·출력 CSV 위치
OUT = FORECAST_DIR
ASOS_FILE = FORECAST_DIR / "honam_asos24_2025.csv"
...
def load_honam_solar() -> pd.DataFrame:
    ...
    df = pd.read_csv(
        FORECAST_DIR / "2. 한국전력거래소_지역별 시간별 태양광 및 풍력 발전량_2025.csv",
        encoding="cp949")
...
def load_national_demand() -> pd.DataFrame:
    ...
    df = pd.read_csv(FORECAST_DIR / "1. 한국전력거래소_시간별 전국 전력수요량_2025.csv",
                     encoding="cp949")
...
def load_kpx_daily() -> pd.DataFrame:
    """파일 5 → 일별 수급실적 (예비율)."""
    df = pd.read_csv(FORECAST_DIR / "5. KPX전력수급실적_2025.csv", encoding="cp949")
...
def export_node_file(pred: pd.DataFrame, raw: pd.DataFrame) -> None:
    ...
    [date, node, hour, TA, SI, predicted_curtailment_mwh]
    ...
    top5 = tmp.groupby("date")["expected_curtail_mwh"].sum().nlargest(5).index.tolist()
    sub = tmp[tmp["date"].isin(top5)]
    ...
    for node, w in NODE_W.items():
        ...
        t["predicted_curtailment_mwh"] = (
            sub["expected_curtail_mwh"].values * w).round(1)
        rows.append(t.rename(columns={"h": "hour"}))

    node_df = (pd.concat(rows)[["date", "node", "hour", "TA", "SI",
                                "predicted_curtailment_mwh"]]
               .sort_values(["date", "node", "hour"]))
    node_df.to_csv(OUT / "pred_curtailment_node_hourly.csv",
                   index=False, encoding="utf-8-sig")
    print(f"[저장] pred_curtailment_node_hourly.csv "
          f"({len(node_df)}행 = {len(top5)}일 × {len(NODE_W)}지점 × 24h)")
    print(f"  상위 5일: {[str(d) for d in top5]}")
